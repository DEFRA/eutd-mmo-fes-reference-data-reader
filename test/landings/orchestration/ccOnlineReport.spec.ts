...
import { DocumentModel, LandingStatus } from "../../../src/landings/types/document";
import logger from '../../../src/logger'

const sinon = require('sinon');

const getCatchCertsStub = sinon.stub(catchCertQuery, 'getCatchCerts')
const getLandingsStub = sinon.stub(landingsPersistence, 'getLandingsMultiple')
const getVesselIdxStub = sinon.stub(cache, 'getVesselsIdx')
const refreshRiskingStub = sinon.stub(cache, 'refreshRiskingData');
...
describe('When validating an online Catch Certificate', () => {
    ...
    beforeEach(async () => {
        getCatchCertsStub.reset();
        getLandingsStub.reset();
        getVesselIdxStub.reset();
        refreshRiskingStub.reset();
    ...
    describe('When call to update risking data fails', () => {
      ...
      it('will handle error thrown whilst refreshing risking', async () => {
        ...
        getCatchCertsStub.returns([]);
        getLandingsStub.returns([]);
        getVesselIdxStub.returns(vesselIdx);
        refreshRiskingStub.rejects(error);
        await generateOnlineValidationReport(payload);

        expect(loggerErrorStub.called).toBeTruthy();
        expect(loggerErrorStub.calledWith(`[ONLINE-VALIDATION-REPORT][REFRESH-RISKING-DATA][ERROR][${error}]`)).toBeTruthy();
      })
    })
    ...
    describe('When certificate is not valid', () => {
        ...
        it('will return a validation report', async () => {
            ...
            getCatchCertsStub.returns(allCatchCerts);
            getLandingsStub.returns(allLandings);
            getVesselIdxStub.returns(vesselIdx);
            refreshRiskingStub.resolves(null);
            const result = await generateOnlineValidationReport(payload);

            expect(result).toEqual(expectedResult);
            expect(refreshRiskingStub.called).toBeTruthy();
        });
        ...
        it('will call getCatchCerts with the referenced landings', async () => {
            ...
            refreshRiskingStub.resolves(null);
            await generateOnlineValidationReport(payload);
            expect(getCatchCertsStub.getCall(0).args[0]).toStrictEqual(
              { landings: [ {
                dateLanded: '2019-10-06',
