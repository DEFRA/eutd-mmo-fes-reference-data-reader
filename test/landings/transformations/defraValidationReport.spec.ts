...
describe('Mapping data for DEFRA Central Reporting HUB', () => {
  ...
  describe('toCcDefraReport', () => {
    ...
    it('can map just a documentNumber, status, correlationId and requestedByAdmin', () => {
      const res = toCcDefraReport('doc1', correlationId, DocumentStatuses.Draft, requestByAdmin);
    ...
    it('can map a whole CC document', () => {
      const res = toCcDefraReport('GBR-2020-CC-1BC924FCF', correlationId, DocumentStatuses.Draft, requestByAdmin, exampleCc);
      ...
      expect(res).toEqual({
        ...
        documentUri: `${ApplicationConfig.prototype.externalAppUrl}/qr/export-certificates/${exampleCc.documentUri}`,
    ...
    describe('when document status is DELETE', () => {
      ...
      it('will display devolvedAuthority when exporter details are available', () => {
        const result: IDefraValidationCatchCertificate = toCcDefraReport('GBR-2020-CC-1BC924FCF', correlationId, 'DELETE', requestByAdmin, exampleCc);
      ...
      it('will not display devolvedAuthority when exporter details are not available', () => {
        ...
        const result: IDefraValidationStorageDocument = toCcDefraReport('GBR-2020-PS-BA8A6BE06', correlationId, 'DELETE', requestByAdmin, backEndCcWithNoExporterDetails);
  ...
  describe('toDefraCcLanding', () => {
    ...
    it('will map products with no landings', () => {
      ...
      const res = SUT.toDefraCcLanding(product, { vehicle: "truck", exportedFrom: "United Kingdom", cmr: true }, '2023-05-01');
    ...
    it('will return true for isDirectLanding if the transportation vehicle is a direct landing', () => {
      ...
      const res = SUT.toDefraCcLanding(product, { vehicle: "directLanding", exportedFrom: "United Kingdom" }, '2023-05-01');
