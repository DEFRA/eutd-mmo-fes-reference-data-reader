...
describe("Report reSubmitted", () => {
  ...
  describe('when a CC is reSubmitted', () => {
    ...
    beforeEach(() => {
      ...
      mockGetCertificateByDocumentNumberWithNumberOfFailedAttempts = jest.spyOn(CatchCert, 'getCertificateByDocumentNumberWithNumberOfFailedAttempts')
      uuid.mockImplementation(() => 'some-uuid-correlation-id');
    ...
    it('all methods required by resendCcToTrade should be called with the correct data', async () => {
      ...
      await Controllers.resendCcToTrade(data);
    ...
    it('should catch any errors thrown', async () => {
      ...
      const caughtError = await Controllers.resendCcToTrade(data).catch((err) => err)
      expect(caughtError).toBe('error');
