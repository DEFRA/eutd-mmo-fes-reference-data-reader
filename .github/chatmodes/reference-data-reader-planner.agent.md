...
Reference Data Reader** service (Node.js/TypeScript/Hapi.js).
...
- Produce complete implementation plans for Reference Data Reader work (transformations, AJV schema
  validation, MongoDB persistence, Azure Service Bus/Blob integrations, scheduled cron jobs).
...
5. Define impacted files/components and expected changes at a high level — including whether the change
   requires the JSON schema (`/data/schemas/Strategic Reporting/`), the TypeScript interface
   (`src/landings/types/`) and the transformation (`src/landings/transformations/`) to be updated together.
