...
description: "Do thorough, risk-scoped research in the open and align findings to the DEFRA standards precedence (DEFRA > GDS > community) for the MMO FES Reference Data Reader service. Use for the single, risk-scoped Research (§4.2) stage of the working framework — validating APIs, libraries, patterns, security and policy against DEFRA/GDS and framework guidance, and citing sources before a plan is approved or implemented."
argument-hint: "e.g. 'validate the AJV draft-2020 schema approach the planner flagged' or 'research Service Bus session vs correlation-id message routing'"
license: OGL-UK-3.0
metadata:
  author: mmo-fes
  version: "1.0"
...
- **Planner - Reference Data Reader** runs this single research pass for **Complex** work and cites sources
  directly in its plan.
- **Developer - Reference Data Reader** runs this same single pass for **Standard** work (or when invoked
...
correctness** (report/schema transformations, weight/risk calculations), **schema validation** (AJV / JSON
Schema), **persistence** (MongoDB/Mongoose, discriminators), **external integrations** (Azure Service Bus,
Blob Storage, Trade API OAuth2, anti-virus), **scheduled jobs** (node-cron), or a **version-sensitive
API/library**. A cosmetic or well-trodden step needs little or none.
...
- Search **authoritative, current** sources: DEFRA & GOV.UK standards and service manuals, OWASP, the
  framework/library's own docs (Hapi, AJV, Mongoose, `@azure/service-bus`, `moment`), and
...
- [ ] **Structured logging** — `bunyan` with bracketed context tags and `_correlationId` propagation.
- [ ] **Data correctness** — transformations, schema validation and calculations preserve integrity; JSON
      schema, TypeScript interface and transformation stay in sync.
...
**blocked**); send **revise/blocked** items back to the **Planner - Reference Data Reader** rather than
