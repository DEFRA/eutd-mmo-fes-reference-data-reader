  ...
  return doc.exportData.catches.map(cat => {
    ...
    if (doc.__t === 'storageDocument')
      specific = {
        ...
        supportingDocuments: Array.isArray(cat.supportingDocuments) && cat.supportingDocuments.length > 0 ? cat.supportingDocuments.filter((doc: string) => doc && doc.trim()).join(',') : undefined,
      }
